...
export async function fetchRevenue() {
    try {
        ...
        const data = await client.sql<Revenue>`SELECT *
                                               FROM revenue`;
...
export async function fetchCardData() {
    try {
        ...
        const invoiceCountPromise = client.sql`SELECT COUNT(*)
                                               FROM invoices`;
        const customerCountPromise = client.sql`SELECT COUNT(*)
                                                FROM customers`;
        const invoiceStatusPromise = client.sql`SELECT SUM(CASE WHEN status = 'paid' THEN amount ELSE 0 END)    AS "paid",
